// DLC v2 — Shared components

const { useState, useEffect, useRef, useCallback, useMemo } = React;

// Stripe placeholder packshot (since we don't have real product photos).
// Renders the brand initials in mono and uses a per-product warm tint.
function Packshot({ product, size = 96, radius = 10, hint = true, missing = false }) {
  if (missing) {
    return (
      <div style={{
  ...
  // Real product image when imageUrl is set
  if (product?.imageUrl) {
    return (
      <div style={{
        width: size, height: size, borderRadius: radius,
  ...
  // Map product → Notion pastel tint by category
  const catTint = {
  ...
  return (
    <div style={{
      ...
      {/* faint stripe to flag this is a placeholder */}
      <div style={{
...
// Copyable field — single row, label on top, value + copy btn.
function CopyField({ label, value, mono = false, large = false, onCopy }) {
...
// Mobile list row (catalogue)
function ProductRow({ product, onClick, dense = false, recent = false }) {
  return (
...
function Pill({ tone = 'neutral', children, size = 'sm' }) {
  // Notion uses small rounded tags (6px radius), not pills
  const tones = {
...
Object.assign(window, { Packshot, CopyField, ProductRow, SideItem, Pill });
