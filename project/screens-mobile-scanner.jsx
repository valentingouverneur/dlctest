// DLC v2 — Scanner screens (3 variants)
// All variants share: full-bleed dark camera bg, viewfinder corners,
// detection states (idle / scanning / detected), session count, close, flash.

const { useState: sUseState, useEffect: sUseEffect } = React;

// Simulated camera background — warm subtle gradient suggesting depth
function CameraBG({ blur = 0 }) {
  return (
    <div style={{
      ...
      {/* Faint shelf/product silhouettes */}
      <div style={{
      ...
      {/* Mock product label visible in viewport center */}
      <div style={{
...
// "Corners only" viewfinder — used in all variants; size & color vary.
function ViewfinderCorners({ w = 280, h = 160, c = '#fff', thickness = 2, len = 22, animated = false }) {
  const corner = (style) => (
    <div style={{
      position: 'absolute', width: len, height: len,
      ...style,
    }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: style.borderRight ? 0 : 'auto', width: style.borderRight ? thickness : len, height: thickness, background: c, borderRadius: 1 }}/>
      <div style={{ position: 'absolute', top: 0, left: 0, bottom: style.borderBottom ? 0 : 'auto', width: thickness, height: style.borderBottom ? len : len, background: c, borderRadius: 1 }}/>
    </div>
  );
  return (
    <div style={{
...
// ─── Variant A — Sobre (clean, minimal, iOS-feel) ──────────────────
function ScannerSobre({ onClose, onDetected, scanCount = 12, state = 'scanning' }) {
  return (
    <div style={{
      position: 'absolute', inset: 0,
      ...
      {/* Dim mask except the viewfinder area */}
      <div style={{
        position: 'absolute', inset: 0,
        background: `
          radial-gradient(ellipse 180px 110px at 50% 50%, transparent 0%, transparent 65%, rgba(0,0,0,0.5) 100%)
        `,
        pointerEvents: 'none',
      }}/>
...
// ─── Variant B — Informative (with stats, history strip) ──────────
function ScannerInformative({ onClose, scanCount = 12, lastScans = [], batchMode = false, onToggleBatch, lowConfidence = false, lowConfidenceEAN = '3033710065●●4', onConfirmLow, onRetry, onPasteEAN, showPaste = false, batchToast = null }) {
  const [paste, setPaste] = sUseState('');
  return (
    <div style={{
      position: 'absolute', inset: 0,
      background: '#0A0908', overflow: 'hidden',
      ...
      {/* Top bar — full width with session info */}
      <div style={{
        position: 'absolute', top: 59, left: 0, right: 0, zIndex: 10,
      }}>
        <div style={{
      ...
      {/* Low-confidence confirm overlay */}
      {lowConfidence && (
        <div style={{
      ...
      {/* Batch mode toast (small, transient) */}
      {batchToast && (
        <div style={{
      ...
      {/* Detected hint floating above viewfinder */}
      ...
      {/* Bottom — paste EAN field (when showPaste) OR recent strip */}
      {showPaste ? (
        <div style={{
...
// ─── Variant C — Minimaliste (full-bleed, accent) ──────────────────
function ScannerMinimal({ onClose, scanCount = 12 }) {
  return (
    <div style={{
      position: 'absolute', inset: 0,
      background: '#0A0908', overflow: 'hidden',
      ...
      {/* Full-bleed accent corners with thinner stroke */}
...
window.ScannerSobre = ScannerSobre;
window.ScannerInformative = ScannerInformative;
window.ScannerMinimal = ScannerMinimal;
window.ViewfinderCorners = ViewfinderCorners;
window.CameraBG = CameraBG;
