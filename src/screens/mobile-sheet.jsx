...
import { MobileBar } from './mobile-catalogue';
import { CameraBG } from './mobile-scanner';

export function Sheet({ children, maxHeight = 720, animate = true }) {
  return (
    <div style={{
      position: 'absolute', bottom: 0, left: 0, right: 0,
      maxHeight: maxHeight === 720 ? '90%' : maxHeight,
...
export function DetectedBadge({ ean }) {
  return (
    <div style={{
...
export function ProductSheetContent({ product, onClose, fromScan = false, onEdit, embedded = false }) {
...
export function FicheMobile({ product, onBack, onScan }) {
  return (
    <div className="scr">
      <MobileBar
        title=""
...
export function Field({ label, value, placeholder, mono, readOnly }) {
  return (
    <div>
      <div style={{
        fontSize: 11, fontWeight: 600, letterSpacing: '0.06em',
...
export function FieldChips({ label, options, active }) {
...
export function NotFoundMobile({ ean = '3017624999999', onClose, onCreate }) {
  return (
    <div style={{
...
export function MultipleResultsMobile({ ean, results, onPick, onClose }) {
  return (
        ...
        <div style={{ padding: '8px 0' }}>
          {results.map(p => (
            <div key={p.ean + p.weight} style={{
              ...
              <Packshot product={p} size={48} radius={6} hint={false}/>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: 14, fontWeight: 500, color: 'var(--ink)',
                  overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                }}>{p.title}</div>
                <div style={{ fontSize: 12, color: 'var(--ink-4)', marginTop: 2, display: 'flex', gap: 6 }}>
                  <span style={{ color: 'var(--ink-3)' }}>{p.brand}</span>
                  <span>·</span>
...
export function BurgerMobile({ onClose, onOpen }) {
  const items = [
...
export function DLCMobile({ items, onBack }) {
